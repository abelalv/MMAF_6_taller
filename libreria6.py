...
class CellularGrowthSimulatorFixed:
    """
    Simulador interactivo del crecimiento de células cancerígenas con tratamiento
    Versión con N₀ fijo en 50
    """
    
    def __init__(self, K=800, r0=0.25, alpha0=0.01, Tf=40, Td=3.5):
        """
        Inicializa el simulador con parámetros por defecto
    ...
    def _setup_ui(self):
        """Configura la interfaz de usuario"""
        self.controls = widgets.HBox(
            [self.r_slider, self.alpha_slider], 
            layout=widgets.Layout(flex_flow='row wrap', justify_content='space-around')
        )
    ...
    def _update_plot(self, change):
        ...
        with self.output_plot:
            ...
            ax.set_xlabel('Tiempo (días)')
            ax.set_ylabel('Número de células')
            ax.set_title('Crecimiento Celular: N₀ = 50 (Fijo)')
            ax.legend()
            ax.grid(True, alpha=0.3)
            ax.set_ylim(0, self.K * 1.1)
            ax.set_xlim(0, self.Tf)
    ...
    def show_simulation(self):
        ...
        print("Simulación de Crecimiento Celular con Tratamiento")
        print("=" * 50)
        print(f"Parámetros fijos: K = {self.K}, N₀ = {self.N0_fixed}, Tiempo final = {self.Tf} días")
        print("A se calcula automáticamente como (K - N₀)/N₀")
        print()
...
# Función de conveniencia para uso rápido
def mostrar_simulacion_fixed(K=800, r=0.25, alpha=0.01, Tf=40):
    """
    Función rápida para mostrar la simulación con N₀ fijo en 50
